...
from datasets.AuroraTWandERA5TWDatasetforAuroraProb import AuroraTWandERA5TWDatasetforAuroraProb
...
def parse_args():
    parser = argparse.ArgumentParser(description = "Aurora Training Script with Probabilistic Aurora Prediction Input")
...
def create_dataset(args, split):
    if split == "train":
        ds = AuroraTWandERA5TWDatasetforAuroraProb(
...
def train_epoch(
    ...
    # Update aurora probability for this epoch
    dataloader.dataset.set_aurora_prob(aurora_prob)
    logger.info(f"Epoch {epoch} - aurora_prob: {aurora_prob:.4f}")
...
def main():
    ...
    for epoch in range(starting_epoch, args.epochs + 1):
        ...
        if accelerator.is_main_process:
            logger.info(f"epoch {epoch} - train_loss: {train_loss:.8f} - aurora_prob: {aurora_prob:.4f}")
