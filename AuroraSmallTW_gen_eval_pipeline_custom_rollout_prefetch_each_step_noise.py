...
Noise policy: Gaussian noise is injected into the input batch BEFORE EVERY
forward pass (re-sampled each step). The autoregressive update itself is
unchanged — noise enters only through the input the model sees.
"""
...
# -------------------------------------------------------------------
# Noise-injected rollout (every step)
# -------------------------------------------------------------------
...
def rollout_with_gpu_each_step_noise(
        model,
        batch,
        steps,
    ...
    """Re-sample noise and perturb the input before every forward pass."""
...
def parse_args():
    parser = argparse.ArgumentParser(description = "Aurora Evaluation (Single GPU) - Prefetch + Each-Step Noise Ablation.")
    ...
    with torch.inference_mode():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            with context_manager:
                generator = rollout_with_gpu_each_step_noise(
                    model,
                    _input,
...
def main():
    args = parse_args()
    set_seed(args.seed)
    logger.info("Running single-GPU evaluation (prefetch + each-step noise ablation).")
