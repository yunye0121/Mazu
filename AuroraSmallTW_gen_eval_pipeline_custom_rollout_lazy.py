...
# coding=utf-8
"""
Lazy-loading version of AuroraSmallTW_gen_eval_pipeline_custom_rollout.py

Key difference: targets are loaded ONE AT A TIME during the rollout loop,
instead of all at once in the dataset's __getitem__.
This should significantly reduce peak memory usage for long rollouts.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description = "Aurora Evaluation Script (Single GPU) - Lazy Target Loading.")
...
def evaluate(args, model, dataloader, criterion_list, err_agg_list, device):
    ...
    with torch.inference_mode():
        for batch in tqdm(dataloader, desc="Evaluating"):
            # --- Lazy version: batch is (inputs, dates), no labels ---
            inputs, dates = batch
            ...
            with context_manager:
                ...
                for step_index, _pred in enumerate(generator):
                    ...
                    # --- LAZY TARGET LOADING ---
                    # Load only this one target from disk, instead of pre-loading all targets
                    _label_data = dataloader.dataset.load_single_target(
                        base_datetime_strs=dates,  # dates is a tuple of strings, one per sample in batch
                    ...
                    # _pred and _label_data go out of scope here -> can be GC'd
...
def main():
    args = parse_args()
    set_seed(args.seed)
    logger.info("Running single-GPU evaluation (lazy target loading).")
