...
def parse_args():
    ...
    # --- On-the-fly Scheduled Sampling Arguments (Fixed Teacher) ---
    parser.add_argument("--use_onthefly", action="store_true", help="Enable on-the-fly scheduled sampling with fixed teacher")
    parser.add_argument("--mix_ratio_max", type=float, default=0.7, help="Max prob of using teacher output as input")
...
# --- Helper: Mixing Schedule (Fixed Teacher) ---
...
def train_epoch(
        args,
        model,
        teacher_model, # Passed in (fixed, never updated)
        dataloader,
        optimizer,
        scheduler,
        criterion,
    ...
    for batch in pbar:
        ...
        with accelerator.autocast():
            ...
            # Only use fixed Teacher if on-the-fly is ON and randomness hits
            if args.use_onthefly and (random.random() < current_mix_ratio):
                with torch.no_grad():
                    # Fixed Teacher takes t=0 -> Predicts t=1 (Noisy)
...
def main():
    ...
    if args.use_onthefly:
        logger.info("On-the-fly Enabled: Initializing Fixed Teacher Model (weights will NOT be updated)")
