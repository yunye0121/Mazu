        ...
        for batch in tqdm(dataloader, desc = "Evaluating"):
            ...
            if use_amp:
                if args.mixed_precision == "fp16":
                    ...
                    raise Exception("Unsupported mixed precison.")
                with torch.cuda.amp.autocast(dtype = dtype):
                    _ar_preds = [
                        pred for pred in rollout(model, _input, steps = args.rollout_step)
                    ]
            else:
                _ar_preds = [
                    pred for pred in rollout(model, _input, steps = args.rollout_step)
            ...
            for t in range(1, args.rollout_step + 1):
                _pred = _ar_preds[t - 1]
                _label = Batch(
                    surf_vars = _label_list[t - 1]["surf_vars"],
                    atmos_vars = _label_list[t - 1]["atmos_vars"],
                    static_vars = static_data["static_vars"],
                    metadata = Metadata(
                        lat = latitudes,
                        lon = longitude,
