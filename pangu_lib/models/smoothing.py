...
class SegmentedSmoothing(nn.Module):
    def __init__(self, kernel_size: int, boundary_width: int):
        """
        Deprecated, this implementation is slow.
...
class SegmentedSmoothingV2(nn.Module):
    ...
    def forward(self, x_in: torch.Tensor) -> torch.Tensor:
        ...
        x_boundary_count += 1e-3  # Avoid division by zero
...
class LearnedSmoothing(nn.Module):
    def __init__(self, kernel_size: int):
        """
        Uses a learned 2D filter instead of heuristic (avgpool)
        """
        super().__init__()
    ...
    def forward(self, x_in: torch.Tensor) -> torch.Tensor:
        x_1ch = rearrange(x_in, "B C Z H W -> (B C) () Z H W")
        x_out = rearrange(self.conv(x_1ch), "(B C) () Z H W -> B C Z H W", C=x_in.shape[1])
        return x_out
