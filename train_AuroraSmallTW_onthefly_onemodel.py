#!/usr/bin/env python
# coding=utf-8
# Self-feeding scheduled sampling: uses the training model itself (detached)
# to generate input, instead of a fixed teacher model.
...
def parse_args():
    parser = argparse.ArgumentParser(description = "Aurora Training Script (HF Style) - Self-Feeding Scheduled Sampling")
    ...
    # --- On-the-fly Scheduled Sampling Arguments (Self-Feeding) ---
    parser.add_argument("--use_onthefly", action="store_true", help="Enable on-the-fly scheduled sampling (self-feeding)")
    parser.add_argument("--mix_ratio_max", type=float, default=0.7, help="Max prob of using model's own output as input")
...
# --- Helper: Mixing Schedule (Self-Feeding) ---
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        # train_input = x_0 (used to generate self-feeding pred for x_1)
        # train_label slices = x_1 (GT input), x_2 (GT Target)

        train_input, train_label, train_dates = batch

        # train_input is t=0. Label is t=1, t=2.
        ...
        with accelerator.autocast():
            ...
            # ------------------------
            # Input_Prev (t=0) used for self-feeding prediction
            ...
            # Self-feeding: use the training model itself (detached) to generate input
            if args.use_onthefly and (random.random() < current_mix_ratio):
                model.eval()  # Freeze BN running stats & disable dropout
                with torch.inference_mode():
                    # Current model takes t=0 -> Predicts t=1 (detached, no gradient)
                    self_pred_t1 = model(input_prev_step)

                    # Update metadata for the prediction to be treated as input t=1
                    self_pred_t1.metadata.time = input_gt.metadata.time

                    # This is now our input (Detached automatically by inference_mode)
                    final_input = self_pred_t1
                model.train()  # Restore training mode for the actual forward pass
...
def main():
    ...
    # No separate teacher model needed — the training model feeds itself
    ...
    for epoch in range(starting_epoch, args.epochs + 1):
        tl, train_global_step = train_epoch(args, model, train_loader, optimizer, scheduler, AuroraMAELoss, accelerator, epoch, train_global_step)
