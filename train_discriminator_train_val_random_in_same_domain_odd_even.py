...
from datasets.DiscriminatorDataset import (
    ...
    ERA5TWDataset,
    DiscriminatorDataset,              # unused in this script but kept for compatibility
    # SameDomainDiscriminatorDataset,    # <<< NEW: same-domain wrapper
    OddEvenSameSourceDiscriminatorDataset,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=[6],
        help="Hour forecast interval(s) (used for AuroraPredictionDataset)",
    )
    ...
    # <<< NEW: which same-domain experiment to run
    parser.add_argument(
        "--same_domain_type",
        type=str,
        choices=["era5", "aurora"],
        # default="era5",
        required=True,
        help="Same-domain experiment: 'era5' for ERA5 vs ERA5, 'aurora' for Aurora vs Aurora",
    )
...
def main():
    ...
    # ==========================
    # Build SAME-DOMAIN dataset
    #   - ERA5 vs ERA5   (same_domain_type = "era5")
    #   - Aurora vs Aurora (same_domain_type = "aurora")
    # ==========================

    if args.same_domain_type == "era5":
        # Two *independent* ERA5 groups with identical config
        era5_group0 = []
        era5_group1 = []

        # forecast_hour doesn't affect ERA5TWDataset, so we just loop to keep shape identical
        for _ in args.forecast_hour:
            era5_group0.append(
                ERA5TWDataset(
        ...
        # full_ds = SameDomainDiscriminatorDataset(
        #     dataset_group0=era5_group0,   # label 0
        #     dataset_group1=era5_group1,   # label 1
        # )
        # logger.info("Using SAME-DOMAIN dataset: ERA5 vs ERA5")
        full_ds = OddEvenSameSourceDiscriminatorDataset(
            base_datasets=era5_group0,
        )

    elif args.same_domain_type == "aurora":
        # Two *independent* Aurora groups with identical config
        aurora_group0 = []
        aurora_group1 = []

        for s_h in args.forecast_hour:
            aurora_group0.append(
                AuroraPredictionDataset(
        ...
        # full_ds = SameDomainDiscriminatorDataset(
        #     dataset_group0=aurora_group0,   # label 0
        #     dataset_group1=aurora_group1,   # label 1
        # )
        # logger.info("Using SAME-DOMAIN dataset: Aurora vs Aurora")
        full_ds = OddEvenSameSourceDiscriminatorDataset(
            base_datasets=aurora_group0,
        )

    else:
        raise ValueError(f"Unknown same_domain_type: {args.same_domain_type}")
