...
def parse_args():
    ...
    parser.add_argument("--gaussian_noise_std", type = float, default = 0.0, help = "Std dev of Gaussian noise for data augmentation. Set to 0 to disable.")
    parser.add_argument("--gaussian_noise_steps", type=int, default=-1, 
                        help="Apply noise to the last K steps only. Set -1 to apply to all steps.")
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        # --- [START CHANGE] Fixed-Scale Noise Injection ---
        if args.gaussian_noise_std > 0.0:
            with torch.no_grad():
                
                # --- A. Surface Variables ---
                # Surface vars are shape [B, T, H, W]. 
                # The scale is just a single float number from the dict.
                for var_name, tensor in train_input["surf_vars"].items():
                    if var_name in scales:
                        # 1. Get the fixed scale
                        fixed_std = scales[var_name]
                        
                        # 2. Determine steps to noise
                        t_dim = tensor.shape[1]
                        k = args.gaussian_noise_steps
                        if k == -1 or k >= t_dim:
                            steps = t_dim; start = 0
                        else:
                            steps = k; start = t_dim - k
                            
                        # 3. Generate noise
                        noise_shape = list(tensor.shape)
                        noise_shape[1] = steps
                        noise = torch.randn(noise_shape, device=tensor.device, dtype=tensor.dtype) * fixed_std * args.gaussian_noise_std
                        
                        # 4. Inject
                        if start == 0:
                            train_input["surf_vars"][var_name] += noise
                        else:
                            train_input["surf_vars"][var_name][:, start:, ...] += noise
                    else:
                        # Fallback if key missing (optional warning)
                        pass

                # --- B. Atmospheric Variables ---
                # Atmos vars are shape [B, T, Levels, H, W].
                # We need to build a scale tensor of shape [1, 1, L, 1, 1] to match the Levels dimension.
                for var_name, tensor in train_input["atmos_vars"].items():
                    # We construct a list of scales for the specific levels in this batch
                    # e.g., if var_name is 't', we look for 't_50', 't_100', 't_250'...
                    ...
                    if found_levels:
                        ...
                        # Apply scale_tensor (broadcasting will handle the dimensions)
                        noise = torch.randn(noise_shape, device=tensor.device, dtype=tensor.dtype) * scale_tensor * args.gaussian_noise_std
                        
                        # Inject
                        if start == 0:
                            train_input["atmos_vars"][var_name] += noise
                        else:
                            train_input["atmos_vars"][var_name][:, start:, ...] += noise
